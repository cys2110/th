...
const schema = object({
  ...
  start_year: coerce.number().optional(),
  end_year: coerce.number().optional(),
  ...
  try {
    const { data, error } = await supabase
      ...
      .insert({
        ...
        start_year: event.data.start_date?.toDate("UTC").getFullYear() || event.data.start_year,
        end_year: event.data.end_date?.toDate("UTC").getFullYear() || event.data.end_year,
  ...
  () =>
    [
      { label: "Player", key: "player", type: "slot", required: true },
      {
        ...
        type: "radio",
        items: Object.entries(PLAYER_TEAM_RELATIONSHIP_TYPE_MAPPING).map(([key, value]) => ({ label: value, value: key })),
        value: "value"
      },
      { label: "Start Date", key: "start_date", type: "date" },
      { label: "End Date", key: "end_date", type: "date" },
      ...(state.value.start_date ? [] : [{ label: "Start Year", key: "start_year", type: "text", subType: "number" }]),
      ...(state.value.end_date ? [] : [{ label: "End Year", key: "end_year", type: "text", subType: "number" }]),
      ...(state.value.relationship_type === "loan" ? [{ label: "Permanent Team", key: "parent_team", type: "slot" }] : [])
