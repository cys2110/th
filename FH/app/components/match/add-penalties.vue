...
const {
  ...
  async () => {
    const { data, error } = await supabase
      .from("match_lineup")
      .select(
        "team_id, shirt_number, starter, captain, ...position(position:name, position_group:group_name), ...player(id, aka, ...people(full_name, ...country!nationality_country_id(icon)))"
      )
...
const onSubmit = async (event: FormSubmitEvent<Schema>) => {
  ...
  try {
    const { error } = await supabase.from("penalty_shootout_attempts").insert(
      event.data.map(penalty => {
        ...
        return {
          ...
          team_attempt_number: Math.ceil(penalty.attempt_number / 2)
        }
      })
    )
