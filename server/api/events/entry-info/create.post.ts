export default defineEventHandler(async event => {
  ...
  switch (params.relationship) {
    ...
    case "Walkover":
      ...
      params["teammate"] = params.teammate ?? null
        ...
        CALL (*) {
          WHEN $draw = 'Main' THEN {
            WHEN $relationship = 'Default' THEN {
              ...
              SET t.reason = $reason, t.teammate = $teammate, s.incomplete = 'Def'
            }
            WHEN $relationship = 'Retirement' THEN {
              MATCH (f)-[:SCORED]->(s:Loser)
              MERGE (f)-[t:RETIRED]->(e)
              SET t.reason = $reason, t.teammate = $teammate, s.incomplete = 'R'
            }
