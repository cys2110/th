export default defineEventHandler(async event => {
  ...
  const { records: h2hRecords } = await useDriver().executeQuery(
    `/* cypher */
      ...
      WITH apoc.map.merge(apoc.map.submap(opponent, ['id', 'first_name', 'last_name'], null, false), {country: properties(c)}) AS opponent, count(m) AS matches, SUM(CASE WHEN s:Winner THEN 1 ELSE 0 END) AS wins
      WHERE matches > 0
