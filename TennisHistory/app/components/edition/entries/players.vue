...
import {
  ...
  LazyEditionEntriesCountryCreate,
  LazyEditionEntriesLcCreate,
  LazyScrapeActivity,
  UBadge,
  UButton,
  UFieldGroup
...
const {
  ...
  async () => {
    ...
    const { data, error } = await supabase
      ...
      id,
      countries(*),
      players!inner(id, first_name, last_name, full_name, tour),
      rank,
      entries!inner(
        id,
        points,
    ...
    for (const pem of Object.values(groupedPems)) {
      ...
      players.push({
        ...
        full_name: pem[0]?.players.full_name!,
...
const columnVisibility = ref({
  ...
  singles_checkbox: isAdmin.value,
  ...
  doubles_checkbox: isAdmin.value,
    ...
    ...(isAdmin.value && {
      footer: () =>
        h(UFieldGroup, { class: "w-fit mx-auto" }, () => [
          h(
            id === "9210" ? LazyEditionEntriesLcCreate
            : COUNTRY_DRAWS.includes(id) ? LazyEditionEntriesCountryCreate
            : LazyEditionEntriesCreate,
...
const handleSelectRow = (_e: Event, row: TableRow<IndividualPlayerEntryInterface>) => {
  if (!isAdmin.value) {
    router.push({
      ...
      params: {
        id: row.original.id,
        name: kebabCase(row.original.full_name)
      }
    })
  }
}
