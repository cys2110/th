...
const {
  ...
  async () => {
    ...
    const { data: withdrawals, error: withdrawalsError } = await supabase
      .from("withdrawals")
      .select("entries(player_entry_mapping(player_id))")
      .eq("event_id", `${edId}-Country`)
        ...
        players: entry.player_entry_mapping.map(
          player =>
            ({
              ...
              withdrawn: !!withdrawals?.find(withdrawal => withdrawal.entries.player_entry_mapping.find(pem => pem.player_id === player.players.id))
            }) as CountryTeam
        )
      }))
      .sort((a, b) => {
      ...
      <template #description>
        ...
        >
          ...
          <template #full_name-cell="{ row }">
            <u-link
              :to="{ name: 'player', params: { id: row.original.player_id, name: kebabCase(row.original.full_name) } }"
