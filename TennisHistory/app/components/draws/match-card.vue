...
<template>
  <div
    ...
    :class="{
      even: isEven,
      odd: !isEven,
      highlighted: match.sides?.some(side => side?.entryId === selectedEntryId)
    }"
  >
    <div class="flex w-full max-w-250 justify-center z-2 relative">
      <span class="absolute left-0 pl-1.75 w-full bottom-[calc(100%+3px)] text-muted text-sm mb-1.5">
        {{ match.date ? formatDate(match.date) : "" }}{{ match.date && match.duration ? " | " : "" }}{{ match.duration }}
      </span>
      ...
      <div class="flex-1 grid grid-rows-[minmax(0,1fr)_minmax(0,1fr)] grid-cols-[minmax(0,1fr)] gap-y-0.5">
        <draws-side-row
          ...
          :entries
          :selected-contestant-id="selectedEntryId"
          :handle-highlight-path
          :isByeMatch="match.incomplete === 'B'"
        />
        <draws-side-row
          :side="match.sides?.[1]"
      ...
      <div
        ...
        :class="{
          'group-[.odd.highlighted]:shadow-[2px_0_0_0_var(--ui-primary)]':
            (match.sides?.[0]?.isWinner && match.sides?.[0]?.entryId === selectedEntryId) ||
            (match.sides?.[1]?.isWinner && match.sides?.[1]?.entryId === selectedEntryId)
        }"
      ></div>
      <div
