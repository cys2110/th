...
const { data: rounds, pending } = await useAsyncData<Array<Round>>(
  key,
  async () => {
    ...
    return data.map((roundData: any) => {
      ...
      return {
        ...
        matches: matches.map((match: any) => {
          const { team1, team2, match_type, winner_id, match_scores, match_stats, team_1_id, team_2_id, match_no } = match
