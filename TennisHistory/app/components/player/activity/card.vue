...
<template>
  <u-page-card
    ...
    :ui="{ container: 'lg:grid-cols-5', wrapper: 'lg:col-span-2', title: 'text-lg', description: 'text-sm', footer: 'flex flex-wrap gap-2' }"
  >
    <template #leading>
      <u-badge
        :color="event.level"
    ...
    <u-table
      ...
      :meta="{
        class: {
          tr: (row: TableRow<ActivityMatchType>) =>
            `${row.original.draw === 'Main' ? '' : 'bg-elevated'} ${row.original.stats ? '' : 'cursor-default! hover:bg-default!'}`
        }
      }"
    />
  </u-page-card>
