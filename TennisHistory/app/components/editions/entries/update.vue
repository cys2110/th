...
const { entry, player, refresh, type } = defineProps<{
  ...
  player?: PersonType & { tour: TourEnumType }
...
const initialState = {
  ...entry,
  tour: player?.tour ? (tourEnum[player.tour as keyof typeof tourEnum] as TourEnumType) : undefined,
  edition: Number(edId),
  type,
  id: player?.id,
...
const onSubmit = async (event: FormSubmitEvent<EntryFormSchema>) => {
  ...
  if (Object.keys(dirtyFields).length) {
    ...
    const response = await $fetch(`/api/editions/entries/${entry ? "update" : "create"}`, {
      method: "POST",
      body: dirtyFields
    })
