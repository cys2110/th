export default defineEventHandler(async event => {
  ...
  let query = `/* cypher */
    ...
    WITH [x IN labels(p1) WHERE x IN ['ATP', 'WTA']][0] AS p1Tour, properties(c1) AS p1Country, apoc.map.clean(apoc.map.submap(p1, ['id', 'first_name', 'last_name', 'pm', 'rh', 'bh', 'dob', 'height', 'ch_singles', 'ch_doubles'], null, false), [], [null]) AS p1Details, p1

    WITH apoc.map.merge(p1Details, {tour: p1Tour, country: p1Country}) AS player1, p1
  `
  ...
  if (team1Ids.length === 2) {
    ...
    query += `/* cypher */
      ...
      WITH [x IN labels(p2) WHERE x IN ['ATP', 'WTA']][0] AS p2Tour, properties(c2) AS p2Country, apoc.map.clean(apoc.map.submap(p2, ['id', 'first_name', 'last_name', 'pm', 'rh', 'bh', 'dob', 'height', 'ch_singles', 'ch_doubles'], null, false), [], [null]) AS p2Details, player1, p1, p2

      WITH apoc.map.merge(p2Details, {tour: p2Tour, country: p2Country}) AS player2, player1, p1, p2
    ...
    query += `/* cypher */
      MATCH (p1:Player)-[:ENTERED]->(f:Singles)
    `
  }
  ...
  query += `/* cypher */
    ...
    WITH player1,
      ...
      COUNT(DISTINCT CASE WHEN s:Winner AND e:Event THEN s END) AS tour_wins,
      COUNT(DISTINCT CASE WHEN s:Loser AND e:Tour THEN s END) AS tour_losses,
