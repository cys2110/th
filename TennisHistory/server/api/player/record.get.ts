...
export default defineEventHandler(async event => {
  try {
    ...
    const { records: singlesRecords } = await useDriver().executeQuery(query, { ...params, type: "Singles" })
    const { records: doublesRecords } = await useDriver().executeQuery(query, { ...params, type: "Doubles" })
