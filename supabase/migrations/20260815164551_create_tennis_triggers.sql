...
AFTER INSERT ON tennis.events
FOR EACH ROW
EXECUTE FUNCTION tennis.create_laver_cup_rounds();

CREATE OR REPLACE FUNCTION tennis.set_entry_country_id()
RETURNS TRIGGER
LANGUAGE plpgsql
AS $$
DECLARE
...
BEGIN
    ...
    INTO event_date
    FROM tennis.entries e
    JOIN tennis.events ev ON ev.id = e.event_id
    LEFT JOIN tennis.editions ed ON ed.id = ev.edition_id
    WHERE e.id = NEW.entry_id;

    -- Prefer the player's historical nationality for the event date. If no
    -- dated record matches, use the player's current nationality.
    SELECT COALESCE(
        (
            SELECT pnm.country_id
            FROM tennis.prev_nationality_mapping pnm
            WHERE pnm.player_id = NEW.player_id
              AND event_date >= pnm.start_date
              AND event_date < pnm.end_date
            ORDER BY pnm.start_date DESC
            LIMIT 1
        ),
        pe.nationality_id
    )
...
BEFORE INSERT ON tennis.player_entry_mapping
FOR EACH ROW
EXECUTE FUNCTION tennis.set_entry_country_id();

-- Update existing player entries when a historical nationality is added.
CREATE OR REPLACE FUNCTION tennis.update_entry_country_ids_from_nationality()
RETURNS TRIGGER
LANGUAGE plpgsql
AS $$
BEGIN
    UPDATE tennis.player_entry_mapping pem
    SET country_id = NEW.country_id
    FROM tennis.entries e
    JOIN tennis.events ev ON ev.id = e.event_id
    LEFT JOIN tennis.editions ed ON ed.id = ev.edition_id
    WHERE pem.entry_id = e.id
      AND pem.player_id = NEW.player_id
      AND COALESCE(ev.start_date, ed.start_date) >= NEW.start_date
      AND COALESCE(ev.start_date, ed.start_date) < NEW.end_date;

    RETURN NEW;
END;
$$;

CREATE TRIGGER update_entry_country_ids_from_nationality_trigger
AFTER INSERT ON tennis.prev_nationality_mapping
FOR EACH ROW
EXECUTE FUNCTION tennis.update_entry_country_ids_from_nationality();

-- Trigger function to update winner on bye matches
CREATE OR REPLACE FUNCTION tennis.update_bye_winners()
RETURNS TRIGGER
LANGUAGE plpgsql
AS $$
BEGIN
...
BEFORE INSERT OR UPDATE ON tennis.matches
FOR EACH ROW
EXECUTE FUNCTION tennis.update_bye_winners();

CREATE OR REPLACE FUNCTION tennis.update_match_entries()
RETURNS TRIGGER
LANGUAGE plpgsql
AS $$
BEGIN
...
CREATE OR REPLACE FUNCTION tennis.update_tiebreaks()
RETURNS trigger
LANGUAGE plpgsql
AS $$
DECLARE
...
BEFORE INSERT ON tennis.match_scores
FOR EACH ROW
EXECUTE FUNCTION tennis.update_tiebreaks();
