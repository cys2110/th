...
CREATE TABLE tournaments (
    ...
    name TEXT NOT NULL,
    website TEXT,
    tours tour_enum[] NOT NULL DEFAULT []::tour_enum[],
    established INTEGER,
    abolished INTEGER,
...
CREATE TABLE player_coach_mapping (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    player_id TEXT NOT NULL REFERENCES players (id) ON UPDATE CASCADE ON DELETE CASCADE,
    coach_id UUID NOT NULL REFERENCES people (id) ON DELETE CASCADE,
    years TEXT,
...
CREATE TABLE rounds (
    ...
    CONSTRAINT rounds_unique UNIQUE (event_id, tour, match_type, round)
);
...
CREATE TABLE ties (
    ...
    CONSTRAINT ties_unique UNIQUE (round_id, winner_id, loser_id)
);
