...
const selectedTour = ref(tournamentStore.tours)
  ...
  uniqueCountries.value.forEach((country, index) => {
    ...
    uniqueYears.value.forEach(year => {
      const wins = props.winners.filter(w => {
        ...
        const isTourMatch = selectedTour.value.includes(w.tour)
        const isTypeMatch =
          selectedType.value === "Singles" ? w.type === "Singles"
          : selectedType.value === "Doubles" ? w.type === "Doubles"
          : true
