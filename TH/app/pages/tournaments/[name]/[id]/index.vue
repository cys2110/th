...
const tabItems = computed<Array<TabsItem>>(() => [
  ...
  { label: "By the Numbers", value: "numbers", icon: ICONS.stats, disabled: COUNTRY_DRAWS.includes(route.params.id) || route.params.id === "9210" }
])

const breadcrumbs: Array<BreadcrumbItem> = [{ label: "Tournaments", to: { name: "tournaments" } }]

const { data: tournament } = await useAsyncData(route.params.id, async () => {
  const { data, error } = await supabase.from("tournaments").select("*").eq("id", Number(route.params.id)).single()
...
const tournamentYears = computed(() => {
  if (tournament.value?.established) {
    let years = String(tournament.value.established)

    if (tournament.value.abolished && tournament.value.established !== tournament.value.abolished) {
      years += ` - ${tournament.value.abolished}`
    } else {
      years += " - present"
    }
    ...
    <u-page>
      ...
      >
        ...
        <template #description>
          <div v-if="tournamentYears">{{ tournamentYears }} </div>
