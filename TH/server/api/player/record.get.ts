...
export default defineEventHandler(async event => {
  ...
  try {
    const query = `/* cypher */
      ...
      OPTIONAL MATCH (p)-[:ENTERED]->(:Entry)-[:SCORED]->(s:Score)-[:SCORED]->(:$($type))-[:PLAYED]->(r:Round)-[:ROUND_OF]->(e)
      OPTIONAL MATCH (p)-[:ENTERED]->(f:$($type))-[:ALTERNATE]->(e)
      WITH *
    ...
    const { records: singlesRecords, summary: singlesSummary } = await useDriver().executeQuery(query, { ...params, type: "Singles" })
    const { records: doublesRecords, summary: doublesSummary } = await useDriver().executeQuery(query, { ...params, type: "Doubles" })

    const statusObjects = [...formatGqlStatusObjects(singlesSummary.gqlStatusObjects), ...formatGqlStatusObjects(doublesSummary.gqlStatusObjects)]
    ...
    return {
      results: combinedResults.sort((a: any, b: any) => b.year - a.year),
      statusObjects
    }
  } catch (error) {
    ...
    throw createError({
      statusCode: 500,
      statusMessage: `Error fetching ${params.id}'s record`,
