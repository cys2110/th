    ...
    const { data, error } = await supabase
      ...
      .select(
        ...
        *,
        ...
        venue(*, country(*)),
        awards:match_award(*, team(name, short_name), ...player(id, aka, ...people(full_name, ...country!nationality_country_id(icon))))
      `
      )
...
useHead({
  title: () =>
    match.value ?
      `${match.value.home_team.short_name || match.value.home_team.name} v ${match.value.away_team.short_name || match.value.away_team.name} - ${match.value.season.competition.name} ${match.value.season.name}`
    ...
    <u-page>
      <u-page-header :ui="{ title: match ? 'w-full mt-6' : '' }">
        ...
        <template #title>
          ...
          >
            <div>{{ match.home_team.short_name || match.home_team.name }}</div>
            ...
            <div>{{ match.away_team.short_name || match.away_team.name }}</div>
          </div>
      ...
      <u-page-body>
        <template v-if="match">
          <u-page-grid class="md:grid-cols-3 lg:grid-cols-5 px-6">
            ...
            <u-page-feature
              v-if="match.awards[0]"
              :description="match.awards[0].award_type"
