...
const playersQuery = () => {
  ...
  if (positionQuery.value.length) query.in("position", positionQuery.value as any[])
...
watchDeep(
  [() => route.query.position, () => route.query.team, () => route.query.country, () => searchTerm.value],
  () => {
    set(players, [])
    set(offset, 0)
  },
...
const columns: Array<TableColumn<PlayerType>> = [
  ...
  { accessorKey: "position", header: "Current Position" }
]
