...
const selectedTab = ref<"players" | "coaches" | "fixtures" | "overview" | "captains">("players")
...
useHead({
  title: () => `${team.value?.short_name || team.value?.name || startCase(route.params.name as string)}`,
...
<template>
  <u-container>
    <u-page>
      <u-page-header
        :title="team?.short_name || team?.name || startCase(route.params.name as string)"
      ...
      <u-page-body>
        <div v-if="team">
          ...
          <team-captains v-else-if="selectedTab === 'captains'" />

          <team-coaches v-else-if="selectedTab === 'coaches'" />
