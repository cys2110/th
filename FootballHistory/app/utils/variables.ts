...
export const POSITIONS = Object.entries(POSITION_MAPPING).map(([key, value]) => ({ label: value, value: key }))
...
export const REFEREE_TYPE_MAPPING: Record<RefereeType, string> = {
  ...
  reserve_assistant_referee: "Reserve Assistant Referee",
  var: "Video Assistant Referee",
  assistant_var: "Assistant Video Assistant Referee",
  support_var: "Support Video Assistant Referee"
}
...
export const MATCH_AWARDS = Object.entries(MATCH_AWARD_TYPE_MAPPING).map(([key, value]) => ({ label: value, value: key }))
...
export const COACH_ROLES = Object.entries(COACH_ROLE_MAPPING).map(([key, value]) => ({ label: value, value: key }))
...
export const PLAYER_TEAM_RELATIONSHIPS = Object.entries(PLAYER_TEAM_RELATIONSHIP_TYPE_MAPPING).map(([key, value]) => ({
  label: value,
  value: key
}))
