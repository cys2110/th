...
const competitionOptions = computed(() =>
  useArrayUnique(data.value.map(item => item.season_table!.competition.name)).value.sort((a, b) => a!.localeCompare(b!))
)
const seasonOptions = computed(() => {
  if (filters.value.competition) {
    return useArrayUnique(
      data.value.filter(item => item.season_table!.competition.name === filters.value.competition).map(item => item.season)
...
watch(
  data,
  () => {
    if (!filters.value.competition && data.value.length) {
      filters.value.competition = data.value[0]?.season_table!.competition.name
    }
  },
  { immediate: true }
)
...
watch(
  ...
  () => {
    if (!filters.value.season && filters.value.competition) {
      filters.value.season = data.value.find(item => item.season_table!.competition.name === filters.value.competition)?.season
    }
  },
  { immediate: true }
)
...
const stats = computed(() => {
  const filteredData = data.value.filter(item => {
    const seasonMatch =
      !filters.value.season || (item.season === filters.value.season && item.season_table!.competition.name === filters.value.competition)
