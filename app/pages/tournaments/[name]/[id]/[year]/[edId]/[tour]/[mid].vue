...
useHead({
  title: () => {
    if (match.value) {
      ...
      const t2Name = t2?.team.map((p: any) => `${p.last_name} ${p.last_name}`).join(" / ")

      return `${t1Name} v ${t2Name} | ${tournament} ${year} ${tour}`
    }

    return `${capitalCase(name as string)} ${year} ${tour} ${mid}`
  }
})
    ...
    <u-page>
      ...
      <u-page-header>
        ...
        <template #title>
          ...
          <div v-else> {{ capitalCase(name) }} {{ year }} {{ tour }} {{ mid }} </div>
        </template>
