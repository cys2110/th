...
const initialState = {
  ...
  edition: Number(edId),
  tour:
    tour && "tour" in TourEnum.enum
      ? (tourEnum[tour as keyof typeof tourEnum] as "ATP" | "WTA" | "Men" | "Women")
      : (edition?.tours?.[0] as TourEnumType) || "ATP",
...
const onSubmit = async (e: FormSubmitEvent<EventFormSchema>) => {
  ...
  if (Object.keys(dirtyFields).length) {
    ...
    if ((response as any).ok) {
      toast.add({
        title: `${event?.edition?.tournament?.name ?? edition?.tournament?.name} ${year} ${e.data.tour} ${event ? "updated" : "created"}`,
        icon: icons.success,
        color: "success"
      })
      ...
      if (refresh) {
        ...
        await navigateTo({
          name: "event",
          params: { id, name, edId, year, tour: e.data.tour }
        })
      }
...
<template>
  <u-modal
    :title="event ? `${event.edition?.tournament?.name} ${year} ${tour}` : 'Create Event'"
    v-model:open="open"
  >
    <u-button
      :icon="event ? ICONS.edit : icons.plus"
      :label="event ? `${event.edition?.tournament?.name} ${year} ${tour}` : 'Create Event'"
      block
    />
